<template>
	<view class="uni-searchbar">
		<view :style="{borderRadius:radius+'px',backgroundColor: bgColor}" class="uni-searchbar__box"
			...
			<input v-if="show || searchVal" ref="searchInputRef" :focus="showSync" :disabled="readonly"
				:placeholder="placeholderText" :maxlength="maxlength" class="uni-searchbar__box-search-input"
				type="text" v-model="searchVal" @confirm="confirm" @blur="blur" @focus="emitFocus" />
	...
	export default {
		...
		methods: {
			...
			toFocus() {
				...
				setTimeout(() => {
					that.searchClick()
					if (that.$refs.searchInputRef && that.$refs.searchInputRef.focus) {
						that.$refs.searchInputRef.focus()
					}
				}, 150)
				// #endif
